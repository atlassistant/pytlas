...
class SkillsManager:
  ...
  def get(self):
    ...
    """
    unique_pkgs = list(set(get_package_name_from_module(v.__module__) for v in self._handlers._data.values()))
    self._logger.info(f'Retrieving meta for "{len(unique_pkgs)}" unique packages')
    skills_meta = []

    for pkg in unique_pkgs:
      meta = self._metas.get(pkg, self.lang) or Meta(from_skill_folder(pkg))
  ...
  def install(self, *names):
    ...
    for name in names:
      ...
      self._logger.info(f'Processing "{display_name}" from "{url}"')
      dest = os.path.abspath(os.path.join(self._directory, to_skill_folder(owner, repo)))

      if os.path.isdir(dest):
        self._logger.info(f'Skill "{repo}" already exists, let\'s try an update')
        s, f = self.update(name)
        succeeded.extend(s)
        failed.extend(f)
      else:
        try:
          self._logger.info(f'Starting to clone "{url}" inside "{dest}"')
          ...
          logging.info(f'Successfully installed "{repo}"')
        except subprocess.CalledProcessError as e:
          failed.append(display_name)
          self._logger.error(f'Could not clone the skill repo, make sure you didn\'t mispelled it and you have sufficient rights to clone it. "{e}"')
  ...
  def update(self, *names):
    ...
    for name in to_proceed:
      ...
      self._logger.info(f'Updating "{display_name}"')
      folder = os.path.abspath(os.path.join(self._directory, folder_name))

      if not os.path.isdir(os.path.join(folder, '.git')):
        failed.append(display_name)
        self._logger.warning(f'Not a git repository "{folder}", skipping')
      else:
        try:
          self._logger.info(f'Pulling updates for "{display_name}"')
          subprocess.check_output(['git', 'pull'], cwd=folder, stderr=subprocess.STDOUT)

          self._install_dependencies(folder)

          succeeded.append(display_name)
          self._logger.info(f'Successfully updated "{display_name}"')
        except subprocess.CalledProcessError as e:
          failed.append(display_name)
          self._logger.error(f'Could not pull in "{folder}": "{e}"')
  ...
  def uninstall(self, *names):
    ...
    for name in names:      
      ...
      if not os.path.isdir(folder):
        failed.append(display_name)
        self._logger.warning(f'Skill "{display_name}" does not seems to be installed in "{folder}", skipping')
      else:
        self._logger.info(f'Removing "{folder}"')
        try:
          rmtree(folder)        
          succeeded.append(display_name)
          self._logger.info(f'Successfully uninstalled {display_name}')
        except Exception as e:
          failed.append(display_name)
          self._logger.error(f'Could not delete "{folder}": "{e}"')
  ...
  def _install_dependencies(self, directory):
    ...
    if os.path.isfile(requirements_path):
      self._logger.info(f'Installing dependencies from "{requirements_path}"')
      subprocess.check_output(['pip', 'install', '-r', 'requirements.txt'], 
      stderr=subprocess.STDOUT, cwd=directory)
    else:
      self._logger.info(f'No requirements.txt available inside "{directory}", skipping')
