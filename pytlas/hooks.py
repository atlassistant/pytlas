...
def on(hook_name, package=None):
  """Decorator applied to a function that handle a specific hook.

  Args:
    hook_name (str): Hook name to listen
    package (str): Optional package name (usually __package__), if not given pytlas will try to determine it based on the call stack

  """

  def new(func):
    register(hook_name, func, package or get_caller_package_name() or func.__module__)
    return func
